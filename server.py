...
LOCAL_LEADS=os.path.join(os.path.dirname(os.path.abspath(__file__)),"leads.txt")
...
def require_api_key(f):
    ...
    def dec(*a,**k):
        raw=get_key()
        if not raw:
            return jsonify({"error":"API key required.","signup":"https://trudayz.com/#pricing"}),401
        r=validate_key(raw)
        if not r["valid"]:
            return jsonify({"error":r["reason"]}),403
        request.key_info=r
        return f(*a,**k)
    return dec
def require_admin(f):
...
GOOGLE_CLIENT_SECRET = os.environ.get("GOOGLE_CLIENT_SECRET", "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")
GOOGLE_REDIRECT_URI = os.environ.get("GOOGLE_REDIRECT_URI", "https://trudayz.com/api/auth/callback")
...
def api_calculate():
    try:
        ...
        if days<=0 or days>1000:
            return jsonify({"error":"Days must be 1-1000."}),400
        start=None
...
@require_api_key
def api_usage():
    return jsonify(get_usage(get_key()))
@app.route("/api/capture",methods=["POST"])
def api_capture():
    data=request.get_json(force=True) if request.is_json else request.form.to_dict()
    email=data.get("email","").strip()
    days=str(data.get("days","")).strip() or "unknown"
    source=data.get("source","web")
    if not email or "@" not in email:
        return jsonify({"status":"error","message":"Valid email required."}),400
    result=save_lead(email,days,source)
    if result=="duplicate":
        return jsonify({"status":"error","message":"Already on the list."}),409
    if result=="saved":
        return jsonify({"status":"success","message":"You are on the list."}),200
