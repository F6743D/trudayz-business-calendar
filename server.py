...
# ==============================================================================
# UNIVERSAL PLATFORM ADAPTER (DYNAMIC INSTANTIATION LAYER)
# ==============================================================================
import os
import io
...
def handler(environ, start_response):
    try:
        ...
        if 'CaptureHandler' in globals():
            # Bypass native __init__ validation to eliminate positional argument mismatches
            handler_class = globals()['CaptureHandler']
            handler_instance = handler_class.__new__(handler_class)
            
            # Manually establish core stream variables expected by base HTTP handlers
            ...
            # Execute standard routing loop natively
            if method == 'POST':
                handler_instance.do_POST()
            else:
                handler_instance.do_GET()
...
# ==============================================================================
# UNIVERSAL PLATFORM ADAPTER (EXPLICIT ROUTING COMPATIBILITY LAYER)
# ==============================================================================
import os
import io
...
def handler(environ, start_response):
    try:
        ...
        # Enforce clean slash matching for root level execution paths
        if not path or path == "":
            path = "/"
        ...
        if 'CaptureHandler' in globals():
            ...
            # Reconstruct environment structures explicitly
            handler_instance.request = mock_socket
            handler_instance.client_address = ('127.0.0.1', 8080)
            handler_instance.server = None
            handler_instance.rfile = mock_socket.rfile
            ...
            # Execute processing loop directly
            handler_instance.handle_one_request()
